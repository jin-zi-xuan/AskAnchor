...
当前 MVP 不接 API、不打开新窗口、不弹出解释面板。用户在 AI 回答里选中一段文字后，AskAnchor 会在当前页面生成“追问这一段”按钮，点击后自动把轻量追问 prompt 填入当前 AI 的输入框，并为选中的文字创建可回跳锚点。
...
- 右侧边缘显示轻量入口“AskAnchor”。
- 点击锚点列表中的任意一项，可滚回原选中文字位置并短暂高亮。
- 用户发送追问后，AskAnchor 会自动跳转到刚发送的那条追问问题。
...
6. 需要回到原文时，点击右侧边缘“AskAnchor”，选择对应锚点。
...
- `contentScript.js`: 文本选择监听、原地追问、锚点记录、发送后定位与回跳。
- `background.js`: 预留的 MV3 service worker。
- `styles.css`: 选区按钮、锚点入口、锚点列表、顶部提示和高亮样式。
